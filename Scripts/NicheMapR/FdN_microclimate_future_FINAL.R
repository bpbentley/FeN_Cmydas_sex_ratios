...
doy <- as.matrix(read.csv(file="Future_inputs/doy.csv")[2])
...
TAI <- as.matrix(read.csv(file="Future_inputs/TAI.csv")[2])
...
TMINN <- as.matrix(read.csv(file="Future_inputs/TMINN.csv")[2]) # minimum air temperatures (deg C)
TMAXX <- as.matrix(read.csv(file="Future_inputs/TMAXX.csv")[2]) # maximum air temperatures (deg C)
RHMINN <- as.matrix(read.csv(file="Future_inputs/RHMINN.csv")[2]) # min relative humidity (%)
RHMAXX <- as.matrix(read.csv(file="Future_inputs/RHMAXX.csv")[2]) # max relative humidity (%)
WNMINN <- as.matrix(read.csv(file="Future_inputs/WNMINN.csv")[2]) # min wind speed (m/s)
WNMAXX <- as.matrix(read.csv(file="Future_inputs/WNMAXX.csv")[2]) # max wind speed (m/s)
CCMINN <- as.matrix(read.csv(file="Future_inputs/CCMINN.csv")[2]) # min cloud cover (%)
CCMAXX <- as.matrix(read.csv(file="Future_inputs/CCMAXX.csv")[2]) # max cloud cover (%)
RAINFALL <- as.matrix(read.csv(file="Future_inputs/rain.csv")[2]) # monthly mean rainfall (mm)
TAIRhr <- as.matrix(read.csv(file="Future_inputs/TAIRhr.csv")[2]) # hourly air temperature input (degrees C) (not used if hourly = 0)
RHhr <- as.matrix(read.csv(file="Future_inputs/RHhr.csv")[2]) # hourly relative humidity input (%) (not used if hourly = 0)
WNhr <- as.matrix(read.csv(file="Future_inputs/WNhr.csv")[2]) # hourly wind speed input (m/s) (not used if hourly = 0)
CLDhr <- as.matrix(read.csv(file="Future_inputs/CLDhr.csv")[2]) # hourly cloud cover input (%) (not used if hourly = 0)
SOLRhr <- as.matrix(read.csv(file="Future_inputs/SOLRhr.csv")[2]) # hourly solar radiation input (W/m2) (not used if hourly = 0)
RAINhr <- as.matrix(read.csv(file="Future_inputs/RAINhr.csv")[2]) # hourly rainfall input (mm) (not used if hourly = 0)
ZENhr <- as.matrix(read.csv(file="Future_inputs/ZENhr.csv")[2]) # hourly zenith angle input (degrees) (not used if hourly = 0 or if negative)
IRDhr <- as.matrix(read.csv(file="Future_inputs/IRDhr.csv")[2]) # hourly longwave radiation input (W/m2) (not used if hourly = 0 or if negative)
...
dates <- seq(as.POSIXct("2026-01-01 00:00", tz = "UTC"),
             as.POSIXct("2105-12-31 23:00", tz = "UTC"), by = "hours")
dates <- dates[!grepl("02-29",as.character(dates))] 

soil$DateTime <- dates

write.csv(file = "NicheMapR_out/Microclimate_ERA5_FdN_2026_2105.csv", soil, quote = F)

library(ggpubr)
ggline(soil, x = "DateTime", y = "D50cm", plot_type = "l")

historic <- read.csv(file = "NicheMapR_out/Microclimate_ERA5_FdN_1978_2025.csv")
historic$DateTime <- seq(as.POSIXct("1978-01-01 00:00", tz = "UTC"),
                         as.POSIXct("2025-12-31 23:00", tz = "UTC"), by = "hours")
historic$X <- NULL

test3 <- as.data.frame(rbind(historic, soil))
ggline(test3, x = "DateTime", y = "D50cm", plot_type = "l",
       xlab = "Date", ylab = "Sand Temperature at 50cm (°C)") + 
  geom_vline(xintercept = as.POSIXct("2026-01-01 00:00"), col = "red", lty = 2) +
